...
export default {
  ...
  watch: {
    valuesByStateCode() {
      let minimum = 0;
      let maximum = Math.max.apply(Math, Object.values(this.valuesByStateCode));
      this.distribution = maximum / 5;
    }
  },
  methods: {
    getStyleFor(stateCode) {
      let value = this.valuesByStateCode[stateCode]
      if (value <= this.distribution) {
        return 'fill: #01C7E5';
      }
      else if (value <= this.distribution * 1) {
        return 'fill: #03C2DF';
      }
      else if (value <= this.distribution * 2) {
        return 'fill: #009FB7';
      }
      else if (value <= this.distribution * 3) {
        return 'fill: #02A2BA';
      }
      else if (value <= this.distribution * 4) {
        return 'fill: #028598';
      }
      else {
        return 'fill: #006574';
